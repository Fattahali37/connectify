...
import { Share2, Heart, MessageCircle, Bookmark, MoreHorizontal } from 'lucide-react';
...
function Home() {
  ...
  const ShareMenu = ({ postId }: { postId: number }) => (
    <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-gray-800 ring-1 ring-black ring-opacity-5 z-50">
      <div className="py-1" role="menu">
        ...
        <button className="block w-full px-4 py-2 text-left text-sm text-white hover:bg-gray-700" role="menuitem">
          Copy Link
        </button>
        <button className="block w-full px-4 py-2 text-left text-sm text-white hover:bg-gray-700" role="menuitem">
          Share to Story
  ...
  const OptionsMenu = ({ post }: { post: typeof posts[0] }) => (
    <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-gray-800 ring-1 ring-black ring-opacity-5 z-50">
      <div className="py-1" role="menu">
        <button 
          onClick={() => setFollowing(prev => ({ ...prev, [post.user.username]: !prev[post.user.username] }))}
          className="block w-full px-4 py-2 text-left text-sm text-white hover:bg-gray-700"
        >
          {following[post.user.username] ? 'Unfollow' : 'Follow'} @{post.user.username}
        </button>
        ...
        <div key={post.id} className="bg-gray-900 rounded-xl mb-6 overflow-hidden">
          <div className="p-4 flex items-center justify-between">
            ...
            >
              ...
              />
              <span className="ml-3 font-medium hover:underline">{post.user.name}</span>
            </div>
